...
build-heat-map = (data, colorlist, name)->
...
run = -> 
	...
	build-heat-map data.nobike["aggregate_nobike"], reverse(take 5, colorbrewer[colorscheme][9]), "Total"
	build-heat-map data.nospace["aggregate_nospace"], drop 4, colorbrewer[colorscheme][9], "Total"

	# build-heat-map data.nobike["aggregate_nobike"], colorbrewer["YlOrRd"][3], "Total"
	# build-heat-map data.nospace["aggregate_nospace"], colorbrewer["GnBu"][3], "Total"
...
# color = d3.scale.quantize!domain [-100, 100] .range colorbrewer[colorscheme][9]
# color = d3.scale.quantize!domain [-180, 180] .range colorbrewer[colorscheme][9]
...
flash = -> 
	...
	# rg = d3.max data, -> it.value
	# rg = 10
	# color = d3.scale.quantize!domain [0, rg] .range colorbrewer[colorscheme][5]
	...
	d3.selectAll ".vcircle"
		...
		.style {
			...
			"fill": (it,i)-> 
				if tmpidx is null 
					gglstationsTsv[i].nono.map (st, j)-> 
						if (st.hour is ch and st.week is cw)
							tmpidx := j
				color gglstationsTsv[i].nono[tmpidx].value
...
# 						color stationsTsv[i].nobike[tmpidx].value
		...
		pv
			...
			.on "mousedown", (it, i)-> console.log i
