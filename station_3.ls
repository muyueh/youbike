...
build-heat-map = (data, colorscheme, name)->
	...
	rg = if colorscheme is "YlOrRd" then 1000 else 30000
	# rg = 100
	# 350
...
run = -> 
	stnm = "捷運公館站(2號出口)"
	# console.log stnm
	build-heat-map data.nobike["aggregate_nobike"], "YlOrRd", stnm
	build-heat-map data.nospace["aggregate_nospace"], "GnBu", stnm
		...
		setvoronoi = ->
			it
				...
				.style {
					...
					"fill": (it,i)-> 
						v = null
						stationsTsv[i].nobike.map -> if (it.hour is ch and it.week is cw) then v := it.value
						color v
					# "fill": "white"
					# "opacity": 0.6
					"opacity": 0.7
