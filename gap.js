...
var each, ggl, svg, svg2, scaleX, scaleM, scaleT, dateToMinute, cutDate, datalist, stnm;
...
svg2 = d3.select("body").append("svg").attr("width", ggl.w + ggl.margin.left + ggl.margin.right).attr("height", ggl.h + ggl.margin.top + ggl.margin.bottom).append("g").attr("transform", "translate(" + ggl.margin.left + ", " + ggl.margin.top + ")");
...
d3.text("./under_n_over/" + stnm + ".csv", function(err, staionTSV){
  ...
  svg.selectAll("line").data(stationJSON).enter().append("line").attr({
    "x1": function(it){
  ...
  return svg2.selectAll("line").data(stationJSON).enter().append("line").attr({
    "x1": function(it){
      return scaleM(dateToMinute(it.start));
    },
    "y1": function(it){
      return ggl.h / 2 + (it.type === "nospace" ? 30 : 0);
    },
    "x2": function(it){
      return scaleM(it.start.getDate() !== it.end.getDate()
        ? 24 * 60
