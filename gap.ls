...
# ggl.h = 670 - ggl.margin.top - ggl.margin.bottom
ggl.h = 300 - ggl.margin.top - ggl.margin.bottom

# ggl.h = 4670 - ggl.margin.top - ggl.margin.bottom
...
scale-t = d3.time.scale!
	.range [ggl.h, 0]

date-to-minute = -> it.getHours! * 60 + it.getMinutes!

# because the above func will cross date, and if so resulting in a full bar
cut-date = -> 
...
# stnm = "信義廣場(台北101)"
# stnm = "臺灣師範大學(圖書館)"
# stnm = "捷運公館站(2號出口)" #"中山行政中心" #"臺灣師範大學(圖書館)" #"信義廣場(台北101)" #"三張犁" #"捷運公館站(2號出口)" # datalist[0]
err, staionTSV <- d3.text "./under_n_over/" + stnm + ".csv"
...
console.log gaptime
console.log [gaptime.nospace / ttl , gaptime.nobike / ttl, (gaptime.nospace + gaptime.nobike) / ttl]
...
scale-x.domain [0, 24]
# scale-y.domain []
scale-t.domain d3.extent stationJSON, -> it.start


console.log d3.extent stationJSON, -> it.start

o_or_u = -> if it is "nospace" then 15 else 0

# console.log o_or_u "nobike"
...
svg
	...
	.attr {
		"x1": -> scale-m date-to-minute it.start
			# scale-x it.start.getHours!
		"y1": -> scale-t it.start # 10 + (o_or_u it.type)
		"x2": -> scale-m (if it.start.getDate! is not it.end.getDate! then (24 * 60 ) else date-to-minute it.end)			
		# scale-x it.end.getHours!
		"y2": -> scale-t it.start # 10 + (o_or_u it.type)
	}
	.style {
		"stroke": -> if it.type is "nospace" then "orange" else "red"
		"stroke-width": 2px
		# "opacity": 0.5
	}
	.on "mouseover" -> console.log [it.start, it.end]
...
svg2
	...
	.attr {
		"x1": -> scale-m date-to-minute it.start
			# scale-x it.start.getHours!
		# "y1": -> scale-t it.start # 10 + (o_or_u it.type)
		"y1": -> ggl.h / 2 + (if it.type is "nospace" then 30 else 0)
		"x2": -> scale-m (if it.start.getDate! is not it.end.getDate! then (24 * 60 ) else date-to-minute it.end)			
		# scale-x it.end.getHours!
		# "y2": -> scale-t it.start # 10 + (o_or_u it.type)
		"y2": -> ggl.h / 2 + (if it.type is "nospace" then 30 else 0)
	}
	.style {
		"stroke": -> if it.type is "nospace" then "orange" else "red"
		"stroke-width": 30
