...
d3.json("./week_hour_total/station_start.json", function(err, startJson){
  var enddate, at, datalist, hdata, wdata, buildHeatMap, run, ls, fwait;
  ...
  ls = ["neg", "pos", "sum"];
  fwait = ls.length;
  return ls.map(function(it, j){
    data[it] = {};
    return d3.tsv("./move_week _hour/" + it + ".tsv", function(err, temp){
      var wait;
      --fwait;
      wait = temp.length;
