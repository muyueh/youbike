...
var ref$, each, listsToObj, ggl, datalist, gglstationsTsv, cw, ch, hdata, wdata, data, color, pie, rds, arc, svg, pieColor, buildPie, buildHeatMap, parse, run, lsfl, wait, flokay, w, h, mapOffset, styleName, zoomLevel, colorscheme, wtbl, dur, move, flash, overlayGoog;
...
run = function(){
  ...
  buildHeatMap(data.nobike["aggregate_nobike"], "YlOrRd", "Total");
  return buildHeatMap(data.nospace["aggregate_nospace"], "GnBu", "Total");
  function fn$(){
    var i$, to$, results$ = [];
...
flash = function(){
  var rg, color, tmpidx, l, nobike, nospace, good, ttl, nobike_p, nospace_p, good_p, up;
  rg = 10;
  color = d3.scale.quantize().domain([0, rg]).range(colorbrewer[colorscheme][3]);
  ...
  d3.selectAll(".vcircle").transition().duration(1000).style({
    "fill": function(it, i){
      if (tmpidx === null) {
        gglstationsTsv[i].nobike.map(function(st, j){
          if (st.hour === ch && st.week === cw) {
            return tmpidx = j;
          }
        });
      }
      return color(gglstationsTsv[i].nobike[tmpidx].value);
    }
  });
