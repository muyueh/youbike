...
var each, projection, padding, w, h, topGraph, lngDim, latDim, styledMap, map, parsing, datalist;
...
parsing = function(csvString){
  var result;
  result = [];
  d3.csv.parseRows(csvString).map(function(it){
    var t, w, r;
    t = new Date(it["1"]);
    w = t.getDay();
    r = {
...
d3.text("./data/信義廣場(台北101).csv", function(err, csvData){
  var parseddata, weekDayTable, barPerMonth, barPerWeekDay, barPerHour, ndx, all, monthDim, dayDim, weekdayDim, hourDim, acciMonth, acciDay, acciWeekDay, acciHour, deathMonth, deathDay, deathWeekDay, deathHour, barWidth, barHeight, marginJson, colorBike;
  ...
  dayDim = ndx.dimension(function(it){
  ...
  deathDay = dayDim.group().reduceSum(function(it){
    return it.value;
  });
  ...
  barPerMonth.width(barWidth + 60).height(barHeight).margins(marginJson).dimension(monthDim).group(deathMonth).x(d3.scale.linear().domain([0, 12])).elasticY(true).colors(function(){
    return colorBike;
  }).yAxis().ticks(3);
  barPerWeekDay.width(barWidth).height(barHeight).margins(marginJson).dimension(weekdayDim).group(deathWeekDay).x(d3.scale.ordinal().domain(weekDayTable)).xUnits(dc.units.ordinal).gap(4).elasticY(true).colors(colorBike).yAxis().ticks(3);
  barPerHour.width(barWidth * 2 + 100).height(barHeight).margins(marginJson).dimension(hourDim).group(deathHour).x(d3.scale.linear().domain([0, 24])).elasticY(true).colors(colorBike).yAxis().ticks(3);
  dc.renderAll();
  return d3.selectAll(".notice").text("警告：這裡很快就會沒車了").style({
    "position": "absolute",
    "left": "450px",
    "top": "50px",
    "z-index": 10,
